...
function fireAd() {
  try {
    ...
    if (typeof w.open === "function") {
      // trigger placement interaction for networks that listen to clicks
    }
    // re-inject meta so scanners pick the placement on interaction
    let m = document.querySelector('meta[name="admaven-placement"]')
    if (!m) {
      m = document.createElement("meta")
      m.setAttribute("name", "admaven-placement")
      document.head.appendChild(m)
    }
    m.setAttribute("content", PLACEMENT)
    // dispatch custom event some tags listen for
    window.dispatchEvent(
      new CustomEvent("admaven-show", { detail: { placement: PLACEMENT } }),
    )
  } catch {
    /* silent */
  }
